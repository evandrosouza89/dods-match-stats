...
from .team_action_event_parser import TeamScoresEvent, TeamEnum, CapturedLocEvent, RoundWinEvent, TickScoreEvent
from .world_action_event_parser import MapLoadingEvent, StartedMapEvent, GameOverEvent, RoundStartEvent, \
        WarmupEndsEvent, RoundRestartEvent, WarmupBeginEvent, AxisReadyEvent, AlliesReadyEvent
...
class MatchStateProcessor:
    ...
    def process(self, event):
        if type(event) in (MapLoadingEvent, StartedMapEvent):
            ...
            elif type(event) == TeamScoresEvent:
                if TeamEnum.AXIS.value == event.team:
                    self.__match.team_axis_team_score = event.rounds_won
                    self.__match.team_axis_tick_score = event.tick_points
                    self.__last_team_score_flag = False
                else:
                    if not self.__last_team_score_flag:
                        self.__match.team_allies_team_score = event.rounds_won
                        self.__match.team_allies_tick_score = event.tick_points
                    else:
                        self.__match.team_axis_team_score = event.rounds_won
                        self.__match.team_axis_tick_score = event.tick_points
                    self.__last_team_score_flag = not self.__last_team_score_flag
    ...
    def __stop_match(self):
        ...
        logging.info("[MatchStateProcessor] - Match state cleared.")
