...
while read -r line; do
  ...
  docker run --name dods-match-stats"$port" -P -d -e DMS_SV_IP="$ip" -e DMS_PORT="$port" -e DMS_DB_URL="$DMS_DB_URL" -e DMS_DB_PORT="$DMS_DB_PORT" -e DMS_DB_USR="$DMS_DB_USR" -e DMS_DB_PW="$DMS_DB_PW" -e DMS_DB_SCH="$DMS_DB_SCH" --network="$DMS_NETWORK" dods-match-stats
